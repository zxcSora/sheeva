...
func manageProject(project config.GitlabElement, client *gitlab.Client) error {
	...
	case "present":
		...
		go UploadProjectAvatar(pId, project, client)
		go ManageProjectVariables(pId, project, client)
		go EditProjectSetting(pId, project, client)
		go ManageSchedules(pId, project, client)
		logger.WithFields(logger.Fields{
			"Project": project.Namespace + "/" + project.Name,
			"ID":      pId,
			"State":   project.State,
...
func ManageProjectVariables(projectId int, project config.GitlabElement, client *gitlab.Client) error {
	CleanUnmanagedVariablesProject(projectId, client)
